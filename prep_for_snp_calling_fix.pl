...
#in above setting tmp directory twice, one specific to java and one specific to picard just in case
...
#mark duplicates:
system "java -jar /data/mjohnson/picard/dist/MarkDuplicates.jar I=$sample\_S_RG.bam O=$sample\_S_RG_MD.bam M=metrics_file_$sample.txt ASSUME_SORTED=true VALIDATION_STRINGENCY=LENIENT MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=330 2>>error_log_$sample.txt"; #set this number below result of ulimit -n (in this case /4 b/c runnign 4 jobs in parallel)
#bam index?
...
###################################################################################
#reorder
system "java -jar /data/mjohnson/picard/dist/ReorderSam.jar INPUT=$sample\_S_RG_MD.bam OUTPUT=$sample\_S_RG_MD_R.bam REFERENCE=$reference VALIDATION_STRINGENCY=LENIENT TMP_DIR=/home/zerega/data/kristen/snps/tmp";
#index:
...
#SplitNCigarReads:
system "java -Xmx6g -Djava.io.tmpdir=/home/zerega/data/kristen/snps/tmp -jar /data/mjohnson/GenomeAnalysisTK.jar -T SplitNCigarReads -R $reference -I $sample\_S_RG_MD_R_RUM.bam -o $sample\_S_RG_MD_R_RUM_SC.bam -U ALLOW_N_CIGAR_READS";
#create the targets:
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $reference -I $sample\_S_RG_MD_R_RUM_SC.bam -o indel_Target_Intervals_$sample.intervals -nt 3 2>>error_log_$sample.txt";
#the realignment:
...
push (@sample_list, "realigned_$sample\_triploid.bam");
close ERROR_OUT;
...
push (@cleanup,
	...
	"$sample\_S_RG.bam",
	"$sample\_S_RG_MD.bam",
	"$sample\_S_RG_MD_R.bam",
	"$sample\_S_RG_MD_R.bai",
	"$sample\_S_RG_MD_R_RUM.bam",
	"$sample\_S_RG_MD_R_RUM.bai",
	"$sample\_S_RG_MD_R_RUM_SC.bam",
	"indel_Target_Intervals_$sample.intervals",
	);
foreach $cleanup_file(@cleanup){
	system "lftp sftp://kristen:moraceae\@10.0.7.154 -e 'cd homes/kristen/data/SNP_calling_intermediate_files_fix; put $cleanup_file;bye'";
