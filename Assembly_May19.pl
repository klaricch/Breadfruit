...
# on command line: script <file with names of paired end reads (pairs separated by tabs)>  <name of storage directory>
...
my $column_names= "Read Count,Pairs Survived,%,Forward Only Survived,%,Reverse Only Survived,%,Dropped,%,A-Tails Removed from Forward,A-Tails Removed from Reverse,T-Tails Removed from Forward,T-Tails Removed from Reverse,Small reads removed,Assembly,Number of contigs,Total size of contigs,Longest contig,Shortest contig,Number of contigs > 1K nt,Percentage of contigs > 1K nt,Number of contigs > 10K nt,Percentage of contigs > 10K nt,Number of contigs > 100K nt,Percentage of contigs > 100K nt,Number of contigs > 1M nt,Percentage of contigs > 1M nt,Number of contigs > 10M nt,Percentage of contigs > 10M nt,Mean contig size,Median contig size,N50 contig length,L50 contig count,contig %A,contig %C,contig %G,contig %T,contig %N,contig %non-ACGTN,Number of contig non-ACGTN nt,Contigs with best hit in protein database,Unique peptides found,Average Ortholog hit ratio, Peptides with best hit in assembly,Unique contigs with hits,Reciprocal best hits,Average collapse factor,";
...
########################################################################################################
########################################################################################################
system "Trinity.pl --seqType fq --JM 100G --left A_WT_R1_out.paired_$fileR1 --right A_WT_R2_out.paired_$fileR2 --single A_WT_R1_out.unpaired_$fileR1 A_WT_R2_out.unpaired_$fileR2 --output trinity_out_dir_$fileR1 --full_cleanup --CPU 11"; 
########################################################################################################
########################################################################################################
#
#                                  LONGEST ISOFORM
