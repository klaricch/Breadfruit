...
#takes the ortholog annotation key, pulls out ortholog ID of any gene that matches "MADS" in the best
...
my $name;foreach my $pep_file(@pep_files){
	...
	if (exists $blast_hash{$name}){
		system "cp $pep_file /home/zerega/data/kristen/madsbox_orthologs/";
		system "cp RAxML_bestTree.$name.tree /home/zerega/data/kristen/madsbox_orthologs/";
	}
}
close IN;
exit;
