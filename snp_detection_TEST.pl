...
# this script will run the pre-processing step needed before variant calling (alignment through indel realignment) given a reference assembly and a sample
# on command line: script.pl <reference_assmebly.fast> <list of samples.txt>
# follow this script with "snp_detection_step2.pl" which performs Base Recalibration and completes SNP/INDEL identification

# example sample format: RNA10_H3El_CAGATC_L003
...
###################################################################################
	...
	$full_name_sample = $list_of_samples;
...
#create the targets:
#system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $reference -I $sample\_S_RG_MD_R_RUM_SC.bam -o indel_Target_Intervals_$sample.intervals 2>error_log_$sample.txt";
#the realignment:
