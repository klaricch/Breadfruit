...
#mark duplicates:
#system "java -jar /data/mjohnson/picard/dist/MarkDuplicates.jar I=$sample\_S_RG.bam O=$sample\_S_RG_MD.bam M=metrics_file_$sample.txt ASSUME_SORTED=true VALIDATION_STRINGENCY=LENIENT MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=1000 2>>error_log_$sample.txt"; #set this number below result of ulimit -n (in this case /4 b/c runnign 4 jobs in parallel)
#bam index?
