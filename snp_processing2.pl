...
#create the targets:
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $reference -I $sample\_S_RG_MD_R_RUM_SC.bam -o indel_Target_Intervals_$sample.intervals -nt 18 2>error_log_$sample.txt";
#the realignment:
...
#merge:
system "java -jar /data/mjohnson/picard/dist/MergeSamFiles.jar INPUT=$sample_inputs OUTPUT=UI_merged_samples.bam";
#############TAKE OUT FOLLOWING STEPS
#reorder:
system "java -jar /data/mjohnson/picard/dist/ReorderSam.jar INPUT=UI_merged_samples.bam OUTPUT=merged_samples.bam REFERENCE=$reference";
#index:
...
###################################################################################
###################################################################################
open ERROR, "> error_log_merged_samples.txt";
#create the targets:
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $reference -I merged_samples.bam -o indel_Target_Intervals_merged_samples.intervals 2>error_log_merged_samples.txt";
#the realignment:
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T IndelRealigner -R $reference -I merged_samples.bam -targetIntervals indel_Target_Intervals_merged_samples.intervals -o realigned_merged_samples.bam 2>error_log_merged_samples.txt";
...
###################################################################################
#call all variants:
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T UnifiedGenotyper -R $reference -I realigned_merged_samples.bam -glm BOTH -ploidy 2 -stand_call_conf 20 -stand_emit_conf 20 -o raw_variants_all_0.vcf -nct 18";
#pull out snps to separate file:
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T SelectVariants -R $reference -V raw_variants_all_0.vcf -selectType SNP -o raw_variants_snps_0.vcf";
#pull out indels to separate file:
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T SelectVariants -R $reference -V raw_variants_all_0.vcf -selectType INDEL -o raw_variants_indel_0.vcf";

#mark high confidence snps (hard-filter):
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T VariantFiltration -R $reference -V raw_variants_snps_0.vcf -window 35 -cluster 3 --filterExpression 'QD<2.0||FS>60.0||MQ<40.0||HaplotypeScore>13.0||MappingQualityRankSum<-12.5||ReadPosRankSum<-8.0' --filterName 'my snp filter' -o snps_filtered_0.vcf";
###########
#ALTERNATIVE
############
...
system "/opt/apps/vcftools/bin/vcftools --vcf snps_filtered_0.vcf --out PASSED_SNPs_0 --remove-filtered-all --recode"; #output file will have the prefix "PASSED_SNPs"
#mark high confidence indels (hard-filter):
system "java -jar /data/mjohnson/GenomeAnalysisTK.jar -T VariantFiltration -R $reference -V raw_variants_indel_0.vcf --filterExpression 'QD<2.0||FS>200.0||ReadPosRankSum<-20.0' --filterName 'my_indel_filter' -o indel_filtered_0.vcf";
#write these high confidence indels to a new file:
system" /opt/apps/vcftools/bin/vcftools --vcf indel_filtered_0.vcf --out PASSED_INDEL_0 --remove-filtered-all --recode";
