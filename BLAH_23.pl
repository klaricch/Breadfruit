...
while (<ASSEMBLYLIST>){
	$assembly_name = $_;
	chomp $assembly_name;
	if ($assembly_name=~/(RNA\d+|EW\d+)/){($sample_name)= $assembly_name=~/(RNA\d+|EW\d+)/;# "if" statement to ensure its defined
	...
	system "blastp -evalue .0000000000000000000000001 -db $assembly_name.pep_database -query /home/zerega/data/kristen/carotenoids/AminoAcidSequences_fve_carotenoid_genes_list.txt -outfmt 6 -out pre_fve_carotenoid_blast_$sample_name.txt";
